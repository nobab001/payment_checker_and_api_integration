...
class ConfigProvider extends ChangeNotifier {
  ...
  void startListening() {
    _subs.add(_svc.globalConfigStream().listen((v) {
      _global = v;
      notifyListeners();
    }));
    _subs.add(_svc.apiKeysStream().listen((v) {
      _apiKeys = v;
      notifyListeners();
    }));
    _subs.add(_svc.socialLinksStream().listen((v) {
      _socialLinks = v;
      notifyListeners();
    }));
    _subs.add(_svc.emailConfigStream().listen((v) {
      _emailConfig = v;
      notifyListeners();
    }));
    _subs.add(_svc.paymentSettingsStream().listen((v) {
      _paymentSettings = v;
      notifyListeners();
    }));
    _subs.add(_svc.usersStream().listen((v) {
      _users = v;
      notifyListeners();
    }));
    _subs.add(_svc.smsGatewaysStream().listen((v) {
      _smsGateways = v;
      notifyListeners();
    }));
  }
  ...
  Future<bool> saveGlobal(GlobalConfig cfg) => _save(() => _svc.saveGlobalConfig(cfg));
  Future<bool> saveApiKeys(ApiKeys keys)     => _save(() => _svc.saveApiKeys(keys));
  Future<bool> saveSocialLinks(SocialLinks l) => _save(() => _svc.saveSocialLinks(l));
  Future<bool> saveEmailConfig(EmailConfig c) => _save(() => _svc.saveEmailConfig(c));
  ...
  Future<void> setUserPermissions(String uid,
          {bool? smsEnabled, bool? gmailEnabled}) =>
      _svc.setUserPermissions(uid,
          smsEnabled: smsEnabled, gmailEnabled: gmailEnabled);
}
